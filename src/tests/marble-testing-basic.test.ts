import { concatMap, delay, iif, map, of, switchMap, take, throwError } from 'rxjs';
...
describe('使用 TestScheduler 測試', () => {
  ...
  test('測試 map operator (帶入更複雜的 value)', () => {
    testScheduler.run(helpers => {
      const { cold, expectObservable } = helpers;
